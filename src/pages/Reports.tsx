...
import { TrendingUp, Users, Gift, IndianRupee } from 'lucide-react';
...
export default function Reports() {
    ...
    const fetchNewUsers = async () => {
      try {
        const response = await axios.get(
          `http://localhost:4000/api/admin/report-and-analysis/new-users`,
          {
            params: { days: timeRange }
          }
        );

        // Use type assertion for response.data to resolve 'unknown' error
        const data = response.data as { success?: boolean; newUsers?: number };

        if (data.success) {
          setNewUsers(data.newUsers ?? 0);
        }
      } catch (error) {
        console.log("Failed to fetch new users", error);
      }
    };
    fetchNewUsers();
  }, [timeRange]);
  ...
  useEffect(() => {
    const fetchTotalGifts = async () => {
      try {
        const response = await axios.get(
          "http://localhost:4000/api/admin/report-and-analysis/total-gifts"
        );
        // Use type assertion for response.data to resolve 'unknown' error
        const data = response.data as { success?: boolean; totalGifts?: number };

        if (data.success) {
          setTotalGifts(data.totalGifts ?? 0);
        }
      } catch (error) {
        console.log("Failed to fetch total gifts", error);
      }
    };
    fetchTotalGifts();
  }, []);
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{(totalGifts || 0).toLocaleString()}</p>
          ...
          <div className="space-y-3">
            {earningsLeaderboard.map((streamer, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                {/* Correct Earnings Value */}
                <p className="font-bold text-gray-900">
                  ₹{Number(streamer.earnings || 0).toLocaleString()}
                </p>
