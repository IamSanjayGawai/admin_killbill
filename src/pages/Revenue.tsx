...
export default function Revenue() {
  const [coinToInrRate, setCoinToInrRate] = useState<string>("1");
  const [platformFee, setPlatformFee] = useState<string>("30");
  const [hostFee, setHostFee] = useState<string>("70");
  const [minWithdrawalAmount, setMinWithdrawalAmount] =
    useState<string>("100");
  ...
  const fetchFees = async () => {
    try {
      const res = await axios.get(
        "http://localhost:4000/api/admin/fees"
      );

      if (res.data?.success) {
        const d = res.data.data;
        setCoinToInrRate(d.coinToInrRate.toString());
        setPlatformFee(d.platformFee.toString());
        setHostFee(d.hostFee.toString());
        setMinWithdrawalAmount(d.minWithdrawalAmount.toString());
      }
    } catch (err) {
      console.error("Fetch fees error", err);
      setError("Failed to load revenue settings");
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (Number(platformFee) + Number(hostFee) !== 100) {
        setError("Platform fee + Host fee must equal 100%");
        setLoading(false);
        return;
      }
      ...
      await axios.put(
        "http://localhost:4000/api/admin/fees",
        {
          coinToInrRate: Number(coinToInrRate),
          platformFee: Number(platformFee),
          hostFee: Number(hostFee),
      ...
      alert("Revenue settings updated successfully");
    } catch (err: any) {
      console.error("Save fees error", err);
      setError(
        err?.response?.data?.message ||
          "Failed to save revenue settings"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <Card title="Revenue & Platform Settings">
        <p className="text-sm text-gray-600 mb-4">
          Configure coin value, platform commission and withdrawal rules.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Coin Rate */}
          <Input
            label="Coin to INR Rate"
            type="number"
            step="0.01"
            value={coinToInrRate}
            onChange={(e) => setCoinToInrRate(e.target.value)}
          />
          ...
          <Input
            ...
            onChange={(e) =>
              setMinWithdrawalAmount(e.target.value)
            }
          />
        ...
        <div className="mt-4 p-4 rounded-lg border bg-gray-50">
          ...
          <p className="text-lg font-semibold">
            {Number(platformFee) + Number(hostFee)}%
          </p>
        </div>
