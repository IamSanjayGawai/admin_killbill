  ...
  const handleSaveFees = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put<ApiResponse<any>>(
        "http://localhost:4000/api/admin/fees-management",
        {
          coinToInrRate: parseFloat(feeFormData.coinToInrRate),
          platformFee: platformFeeNum,
          hostFee: hostFeeNum,
          minWithdrawalAmount: parseFloat(feeFormData.minWithdrawalAmount),
        },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
      ...
      if (response.data?.success) {
        ...
        setInitialFeeData(updatedSettings);
        alert("Revenue settings updated successfully!");
      }
    } catch (err: any) {
      console.error("Save fees error", err);
      setFeeError(
        err?.response?.data?.message || "Failed to save revenue settings"
      );
    } finally {
      setFeeLoading(false);
    }
  ...
  const handleResetFees = () => {
    if (initialFeeData) {
      setFeeFormData({
        ...
        platformFee: initialFeeData.platformFee.toString(),
        hostFee: initialFeeData.hostFee.toString(),
  ...
  const feesTab = (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-gray-200">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Platform Fees & Revenue Settings</h2>
